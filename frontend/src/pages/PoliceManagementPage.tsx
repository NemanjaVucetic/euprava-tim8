...
export default function PoliceManagementPage() {
    ...
    async function createPolice() {
        ...
        try {
            await trafficPoliceApi.createPolice({
                                    ...
                                    <div className="flex flex-col items-end gap-2">
                                        ...
                                        <div className="flex gap-1">
                                            <button
                                                onClick={async () => {
                                                    try {
                                                        await trafficPoliceApi.upgradePoliceRank(p.id);
                                                        await loadPolice();
                                                    } catch (e: any) { setError(e?.message || "Cannot upgrade."); }
                                                }}
                                                disabled={p.rank === "HIGH"}
                                            ...
                                            <button
                                                onClick={async () => {
                                                    try {
                                                        await trafficPoliceApi.downgradePoliceRank(p.id);
                                                        await loadPolice();
                                                    } catch (e: any) { setError(e?.message || "Cannot downgrade."); }
                                                }}
                                                disabled={p.rank === "LOW"}
                                                className="rounded-xl border border-slate-700 bg-white/5 px-3 py-1.5 text-xs font-semibold hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed"
                                            >
